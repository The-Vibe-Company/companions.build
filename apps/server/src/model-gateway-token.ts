...
export function mintModelGatewayToken(companionId:string,runId:string,agentSecret:string,expiresAt=Date.now()+MAX_LIFETIME_MS,endpointSecret=''){
 ...
 const claims={companionId,runId,credentialDigest:createHash('sha256').update(agentSecret).digest('hex'),endpointDigest:createHash('sha256').update(endpointSecret).digest('hex'),expiresAt,issuedAt:Date.now()};
...
export function verifyModelGatewayToken(token:string):ModelGatewayClaims|null{
 ...
 try{
  ...
  if(typeof value.endpointDigest!=='string'||!/^[a-f0-9]{64}$/.test(value.endpointDigest))return null;
  return {companionId:value.companionId,runId:value.runId,credentialDigest:value.credentialDigest,endpointDigest:value.endpointDigest,expiresAt:value.expiresAt};
 }catch{return null;}
}
