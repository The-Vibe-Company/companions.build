...
export function SpecialistIdentity({draft,onSaved,onClose}:{draft:SpecialistDraft;onSaved:(draft:SpecialistDraft)=>void;onClose:()=>void}){
 ...
 async function save(event:FormEvent){
  ...
  try{const result=await workspaceApi.updateTemplateDraft(draft.templateId,{expectedGeneration:draft.generation,name:name.trim(),avatar});onSaved(result.draft);onClose();}
