...
export async function migrate(sql = db) {
  ...
  await sql.begin(async tx => {
    ...
    for (const name of ["product.sql", "plugins.sql", "storage-schema.sql", "automations.sql", "triggers.sql", "lifecycle.sql", "billing.sql", "delivery.sql"]) await tx.unsafe(await Bun.file(new URL(`./${name}`,import.meta.url)).text());
