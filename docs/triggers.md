...
`problemPath` is a bounded dot path to a scalar event identity. Sentry defaults to `group.id`; GitHub defaults to `workflow_run.id`; otherwise the payload digest keeps distinct problems separate. Distinct accepted events join an existing queued batch. Once its run is active, at most one queued follow-up batch is created. Several started batches may coexist when earlier tasks are parked in `needs_input`; only queued batches are unique per problem. `processTriggerInbox()` refreshes batch projections, then locks and checks the actual run again inside the grouping transaction after asynchronous filtering. Once admission has begun, later events create a follow-up instead of changing the context already staged for that run.

Disabled triggers and Companions that are retired or awaiting archive reject new webhook receipt. The worker checks that authority before provider reads/filtering, then locks and rechecks it after filtering and before run admission. A batch awaiting admission when authority disappears is finished without a run; its delivery records `trigger_inactive`. Reads and history remain available to the owner. The transaction lock order is Companion, trigger, then batch/run rows.
