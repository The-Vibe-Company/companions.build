...
CREATE TABLE IF NOT EXISTS model_gateway_requests (
 ...
 provider text NOT NULL CHECK(provider IN ('google','anthropic','openai','openrouter','zai','azure')),
...
-- Existing installations keep every request tombstone and gain the Azure route.
...
ALTER TABLE model_gateway_requests ADD CONSTRAINT model_gateway_requests_provider_check
 CHECK(provider IN ('google','anthropic','openai','openrouter','zai','azure'));
