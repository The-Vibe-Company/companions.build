...
function AccountSpecialistCapacity() {
  const [data, setData] = useState<AccountSpecialistLimits | null>(null); const [active, setActive] = useState(""); const [busy, setBusy] = useState(""); const [error, setError] = useState(""); const mounted = useRef(true); const activeEdited = useRef(false);
  const load = useCallback(async () => { try { const result = await workspaceApi.specialistLimits(); if (mounted.current) { setData(result); if (!activeEdited.current) setActive(String(result.limits.active)); setError(""); } } catch (cause) { if (mounted.current) setError(errorText(cause)); } }, []);
  useEffect(() => { mounted.current = true; void load(); return () => { mounted.current = false; }; }, [load]);
  useEffect(() => { if (!data?.requests.some(item => ["queued", "waiting", "admitted", "cancelling"].includes(item.state))) return; const timer = window.setInterval(() => void load(), 2_000); return () => window.clearInterval(timer); }, [data?.requests, load]);
  if (!data) return null;
  async function save(event: FormEvent) { event.preventDefault(); const value = Number(active); if (!Number.isInteger(value) || value < 0) return; setBusy("limit"); setError(""); try { await workspaceApi.updateSpecialistActiveLimit(value); activeEdited.current = false; await load(); } catch (cause) { setError(errorText(cause)); } finally { setBusy(""); } }
