...
export async function listTemplates(ownerId:string,sql:any=db) {
 return sql`SELECT id,name,instructions,avatar,model_id AS "modelId",revision,source_companion_id AS "sourceCompanionId",software_build_id AS "softwareBuildId",snapshot_name IS NOT NULL AS "hasSnapshot" FROM agent_templates WHERE owner_id=${ownerId} ORDER BY created_at,id`;
}
export async function listTemplateRevisions(ownerId:string,templateId:string,sql:any=db) {
 return sql`SELECT r.revision,r.name,r.instructions,r.avatar,r.model_id AS "modelId",r.snapshot_name AS "snapshotName",r.skill_bundle_id AS "skillBundleId",r.software_build_id AS "softwareBuildId",
...
export async function recordTemplateRevision(sql:any,templateId:string) {
 const [row]=await sql`INSERT INTO template_revisions(template_id,revision,owner_id,name,instructions,avatar,model_id,snapshot_name,source_companion_id,skill_bundle_id,software_build_id)
  SELECT id,revision,owner_id,name,instructions,avatar,model_id,snapshot_name,source_companion_id,skill_bundle_id,software_build_id
...
export async function saveTemplate(ownerId:string,input:unknown,sql:any=db) {
 ...
 if(!value.id){
  ...
  ) INSERT INTO template_revisions(template_id,revision,owner_id,name,instructions,avatar,model_id,snapshot_name,source_companion_id,skill_bundle_id,software_build_id)
   SELECT id,revision,owner_id,name,instructions,avatar,model_id,snapshot_name,source_companion_id,skill_bundle_id,software_build_id FROM saved RETURNING template_id AS id,revision`;
  return row;
 }
...
export async function rollbackTemplate(ownerId:string,templateId:string,input:unknown,sql:any=db) {
 ...
 const [row]=await sql`WITH restored AS (
  UPDATE agent_templates current SET name=historical.name,instructions=historical.instructions,avatar=historical.avatar,model_id=historical.model_id,
   snapshot_name=historical.snapshot_name,source_companion_id=historical.source_companion_id,skill_bundle_id=historical.skill_bundle_id,software_build_id=historical.software_build_id,
  ...
  SELECT id,revision,owner_id,name,instructions,avatar,model_id,snapshot_name,source_companion_id,skill_bundle_id,software_build_id FROM restored RETURNING template_id AS id,revision`;
