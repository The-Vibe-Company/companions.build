...
The recipient signs in through Better Auth and must have the exact verified email before `POST /api/deliveries/:id/accept` succeeds. Activation requires an active subscription whenever Stripe billing is configured. Acceptance creates a new Companion identity and a fresh Box from its verified clean software result, or the configured base template when no software was requested. Portable child templates receive new IDs and preserve their declarative profile, bounded child permission, validated portable-skill bundle, and independently granted software result.

Software preparation starts from an operator-registered immutable distribution. Exact public APT/npm roots are resolved and installed on a separate clean build Box without model, provider or browser credentials. The executor journals each external effect; capture is allowed only after the helper verifies and seals the canonical manifest. A verified immutable result and its creator grant are published in the same fenced transaction that marks the build ready. A lost snapshot response is observed by its original name and never recreated blindly.

A delivery pins main and selected specialist software at creation, including older revisions. Both `skillsStatus` and `softwareStatus` must be ready before an invitation is sent or acceptance succeeds. Failed preparation is visible; it never silently falls back to a base image. Acceptance grants the recipient access to the clean immutable result without exposing the creator’s private build. The worker observes completed builds and sends eligible invitations through the shared mail adapter, so hosted workers also need the mail configuration above.
