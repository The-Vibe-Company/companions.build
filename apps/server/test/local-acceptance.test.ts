...
test.skipIf(process.env.RUN_LOCAL_ACCEPTANCE !== "1")("full local path: real Pi files, independent machines, cancel, daemon crash without replay, next work", async () => {
  ...
  try {
    const ada = await createCompanion({ name: "Acceptance Ada", instructions: "Use tools", provider: "local" }); created.push(ada.id);
    const lin = await createCompanion({ name: "Acceptance Lin", instructions: "", provider: "local" }); created.push(lin.id);
    const first = await acceptMessage(ada.id, crypto.randomUUID(), "write-note");
    await until(async () => (await detail(ada.id))!.runs.some((r: any) => r.id === first && r.status === "succeeded"));
    expect(readFileSync(join(dataDir, "agents", ada.id, "workspace/note.txt"), "utf8")).toBe("written by real Pi tools\n");
    const slow = await acceptMessage(ada.id, crypto.randomUUID(), "slow-write");
    await until(() => existsSync(join(dataDir, "agents", ada.id, "workspace/slow-started")));
    await acceptMessage(lin.id, crypto.randomUUID(), "write-note");
    await until(async () => (await detail(lin.id))!.runs[0].status === "succeeded");
    expect((await detail(ada.id))!.runs.find((r: any) => r.id === slow).status).toBe("running");
    await cancel(ada.id);
    await until(async () => (await detail(ada.id))!.runs.find((r: any) => r.id === slow).status === "cancelled");
    expect(existsSync(join(dataDir, "agents", ada.id, "workspace/should-not-exist"))).toBe(false);
    const crash = await acceptMessage(ada.id, crypto.randomUUID(), "crash-after-effect");
    ...
    await until(async () => (await detail(ada.id))!.runs.find((r: any) => r.id === crash).status === "interrupted");
    expect(readFileSync(effectPath, "utf8")).toBe("effect");
    const next = await acceptMessage(ada.id, crypto.randomUUID(), "write-note");
    await until(async () => (await detail(ada.id))!.runs.find((r: any) => r.id === next).status === "succeeded");
    expect(readFileSync(effectPath, "utf8")).toBe("effect");
    expect((await detail(ada.id))!.messages.filter((m: any) => m.role === "assistant")).toHaveLength(2);
  } finally {
