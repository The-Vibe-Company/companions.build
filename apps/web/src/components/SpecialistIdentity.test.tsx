...
it('saves a name and avatar without requiring an agent profile card',async()=>{
 ...
 const fetcher=vi.fn(async(_url:unknown,options?:RequestInit)=>new Response(JSON.stringify({draft:{...draft,...JSON.parse(String(options?.body)),generation:5}})));
 ...
 expect(JSON.parse(String(fetcher.mock.calls[0][1]?.body))).toEqual({expectedGeneration:4,name:'Robin',avatar:{shape:0,color:5,face:0}});
