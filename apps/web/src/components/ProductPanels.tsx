...
import { api, workspaceApi, type AccountUser, type AgentTemplate, type AgentTemplateRevision, type BillingOverview, type Companion, type DeliveryReceived, type DeliverySent, type MaintenanceAction, type MaintenanceCompanion, type MaintenanceDetail } from "@/api";
...
const deliveryState = (item: DeliverySent | DeliveryReceived) => item.status === "accepted" ? "Accepted" : item.status === "revoked" ? "Revoked" : item.skillsStatus === "pending" ? "Preparing skills…" : item.skillsStatus === "error" ? item.skillsError || "Skills could not be prepared" : "Ready for client";
...
export function DeliverySettings({ companionId }: { companionId: string }) {
  const [templates, setTemplates] = useState<AgentTemplate[]>([]); const [selected, setSelected] = useState<string[]>([]); const [email, setEmail] = useState(""); const [maintenance, setMaintenance] = useState(false); const [busy, setBusy] = useState(false); const [notice, setNotice] = useState<{ text: string; kind: "pending" | "ready" | "error" } | null>(null);
  const deliveryIntent = useRef<{ signature: string; id: string } | null>(null);
  useEffect(() => { workspaceApi.templates().then(result => setTemplates(result.templates)).catch(cause => setNotice({ text: errorText(cause), kind: "error" })); }, []);
  async function submit(event: FormEvent) { event.preventDefault(); setBusy(true); setNotice(null); const signature = JSON.stringify([companionId, email.trim().toLowerCase(), [...selected].sort(), maintenance]); if (deliveryIntent.current?.signature !== signature) deliveryIntent.current = { signature, id: crypto.randomUUID() }; try { await Promise.all(selected.map(templateId => workspaceApi.setTemplatePermission(companionId, templateId, 2))); const { delivery } = await workspaceApi.createDelivery({ clientDeliveryId: deliveryIntent.current.id, companionId, clientEmail: email.trim(), templateIds: selected, maintenanceRequested: maintenance }); deliveryIntent.current = null; setEmail(""); setSelected([]); setMaintenance(false); setNotice(delivery.skillsStatus === "pending" ? { text: "Preparing skills… Follow progress in Account.", kind: "pending" } : delivery.skillsStatus === "error" ? { text: delivery.skillsError || "Skills could not be prepared.", kind: "error" } : { text: "Invitation ready.", kind: "ready" }); } catch (cause) { setNotice({ text: errorText(cause), kind: "error" }); } finally { setBusy(false); } }
