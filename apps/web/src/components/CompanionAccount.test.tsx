...
import { AccountProduct, DeliverySettings, DesktopSheet } from "./CompanionAccount";
...
it('retries an uncertain delivery with its immutable identity and only reports persisted readiness',async()=>{
 const bodies:Array<Record<string,unknown>>=[];
 vi.stubGlobal('fetch',vi.fn((_input:RequestInfo|URL,options?:RequestInit)=>{
  bodies.push(JSON.parse(String(options?.body)));
  return bodies.length===1?response({error:'Transport uncertain'},503):response({delivery:{skillsStatus:'pending'}});
 }));
 const actor=userEvent.setup();render(<DeliverySettings companionId="c1"/>);
 await actor.type(screen.getByRole('textbox',{name:'Client email'}),'client@example.com');
 await actor.click(screen.getByRole('button',{name:'Prepare invitation'}));
 await screen.findByText('Transport uncertain');
 expect(screen.getByRole('textbox',{name:'Client email'})).toBeDisabled();
 await actor.click(screen.getByRole('button',{name:'Prepare invitation'}));
