...
export function MailPanel({ companionId, companionName }: { companionId: string; companionName: string }) {
  ...
  useEffect(() => {
    if (!data?.messages.some(message => ["approved", "queued", "sending", "scheduled"].includes(message.state))) return;
  ...
  async function remind(message: MailMessage) {
    ...
    try { await workspaceApi.createRoutine(companionId, { name: `Review email: ${message.subject || "No subject"}`, prompt: `Remind me to review prepared email draft ${message.id}. Do not send it automatically.`, runAt: runAt.toISOString(), enabled: true }); setNotice("Reminder set for tomorrow at 9:00. The draft will not send automatically."); }
    catch (cause) { setError(errorText(cause)); } finally { setBusy(""); }
  }
