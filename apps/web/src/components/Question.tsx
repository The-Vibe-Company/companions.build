...
export function Question({companionId,question,onAnswered}:{companionId:string;question:{id:string;question:string;options:string[];answer?:string|null;runStatus?:string};onAnswered:()=>Promise<void>}){
 ...
 if(resolved || closed) return <section className="question-panel question-panel--answered" aria-label={resolved ? 'Answered question' : 'Closed question'}><strong>{resolved ? 'Answered' : 'Closed'}</strong><p style={{whiteSpace:'pre-wrap',overflowWrap:'anywhere'}}>{question.question}</p>{!resolved && <p>This question is no longer waiting for an answer.</p>}{resolved && <div className="question-answer"><span>Your answer</span><p>{question.answer}</p></div>}</section>;
 return <section className="question-panel" aria-label="Waiting for your answer" aria-busy={busy}><strong>Waiting for your answer</strong><p style={{whiteSpace:'pre-wrap',overflowWrap:'anywhere'}}>{question.question}</p>
