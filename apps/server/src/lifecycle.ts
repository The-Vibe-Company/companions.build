...
export async function progressLifecycle(sql:any=db,hooks:LifecycleHooks={},machine:LifecycleMachines=machines){
 ...
 for(const companion of await sql`SELECT * FROM companions WHERE retired_at IS NULL AND (prepare_requested OR desktop_taken OR desktop_paused_at IS NOT NULL) ORDER BY created_at LIMIT 50`){
  try {
   if(companion.desktop_paused_at){
    if(!companion.desktop_taken){await machine.pause(companion,false);await sql`UPDATE companions SET desktop_paused_at=null,error=null WHERE id=${companion.id}`;}
    continue;
   }
   if(companion.prepare_requested&&!companion.archive_requested_at){
    if(!companion.preparation_started_at){
     await sql.begin(async(tx:any)=>{await tx`UPDATE companions SET preparation_started_at=now(),create_started_at=COALESCE(create_started_at,now()),status='preparing',error=null WHERE id=${companion.id}`;await usage(tx,companion,'starting');});
     companion.preparation_started_at=new Date();
    }
    if(Date.now()-new Date(companion.preparation_started_at).getTime()>5*60_000){await sql`UPDATE companions SET prepare_requested=false,preparation_started_at=null,status='error',error='Machine preparation timed out. Request preparation to retry.' WHERE id=${companion.id}`;continue;}
    const digest=environmentDigest(companion.agent_secret,companion.provider);
    const endpoint=await machine.prepare(companion,async id=>{await sql`UPDATE companions SET box_id=${id} WHERE id=${companion.id}`;companion.box_id=id;},async()=>{await sql`UPDATE companions SET config_digest=${digest} WHERE id=${companion.id}`;});
    if(!endpoint)continue;
    if(!(await machine.health(endpoint,decrypt(companion.agent_secret)))?.ready)continue;
   ...
   if(companion.desktop_taken&&companion.endpoint_secret&&companion.status==='ready'){
    ...
    await sql`UPDATE companions SET desktop_paused_at=now(),error=null WHERE id=${companion.id}`;
   }
 ...
 for(const candidate of await sql`SELECT k.*,c.box_id,c.provider,c.owner_id FROM template_candidates k JOIN companions c ON c.id=k.source_companion_id WHERE k.status IN ('queued','capturing','ready') AND NOT c.desktop_taken AND c.desktop_paused_at IS NULL ORDER BY k.requested_at LIMIT 50`){
  try{
   if(candidate.status==='queued'){
    ...
    if(exists==='missing')await machine.snapshot(candidate,candidate.snapshot_name);
   }
 ...
 for(const delegation of await sql`SELECT d.*,r.status,r.result_text,r.error,c.owner_id,c.temporary FROM delegations d JOIN runs r ON r.id=d.run_id JOIN companions c ON c.id=d.target_id WHERE d.finished_at IS NULL AND r.status IN ('succeeded','failed','cancelled','interrupted') ORDER BY d.created_at LIMIT 50`){
  ...
  if(!delegation.returned_run_id){
   await sql.begin(async(tx:any)=>{
    ...
    if(!parent){await tx`UPDATE delegations SET result=${result},finished_at=now() WHERE id=${delegation.id}`;if(delegation.temporary)await tx`UPDATE companions SET archive_requested_at=COALESCE(archive_requested_at,now()),prepare_requested=false WHERE id=${delegation.target_id}`;return;}
