...
for (const name of ["launch-headless.py","headless-mounts.sh","desktop-capture.py","desktop-quiesce.py","desktop-state.py","configure-desktop.py","install-desktop.sh"]) digest.update(readFileSync(`${output}/${name}`));
