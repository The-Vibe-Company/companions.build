...
import { ArrowUpRight, Check, CircleDollarSign, CopyPlus, LoaderCircle, MonitorUp, PackageOpen, Pause, Play, Send, ShieldCheck, Trash2, UsersRound, X } from "lucide-react";
import { api, workspaceApi, type AccountUser, type AgentTemplate, type BillingOverview, type Companion, type DeliveryReceived, type DeliverySent } from "@/api";
...
export function DesktopSheet({ companion, onClose, onRefresh }: { companion: Companion; onClose: () => void; onRefresh: () => Promise<void> }) {
  ...
  useEffect(() => {
    if (!waiting) return;
    if ((waiting === "pause" && companion.desktopPausedAt) || (waiting === "release" && !companion.desktopPausedAt) || (waiting === "prepare" && companion.status === "ready")) { setWaiting(""); return; }
  ...
  }, [waiting, companion.desktopPausedAt, companion.status, onRefresh]);
  async function open() { const popup = window.open("about:blank", "_blank"); setError(""); try { const result = await api.openDesktop(companion.id); if (result.url && popup) popup.location.href = result.url; else { popup?.close(); setWaiting("prepare"); await onRefresh(); } } catch (cause) { popup?.close(); setError(errorText(cause)); } }
