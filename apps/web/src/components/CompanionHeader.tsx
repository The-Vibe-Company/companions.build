...
export function CompanionHeader({ detail, section, refreshVersion, onSection, onMenu }: {
  ...
  const activity=finished?"Finished":companion.runtimeUpdateStatus==="updating"?"Updating":companion.runtimeUpdateStatus==="blocked"?"Update needs attention":detail.questions?.some(question=>question.answer==null&&(!question.runStatus||['running','needs_input','preparing'].includes(question.runStatus)))||run?.status==="needs_input"?"Needs you":run?.status==="running"?`${companion.name} · working`:run?.status==="preparing"?"Preparing":run?"Queued":"Idle";
  ...
  return <header className="companion-header">
    ...
    <nav ref={navigation} className="companion-header-links" aria-label="Companion sections">
      ...
      <button className="companion-info-pill" aria-label="Activity" title={companion.runtimeUpdateStatus==="updating"?"Updating this Companion. Your messages are saved and will run when it is ready.":companion.runtimeUpdateStatus==="blocked"?"The update could not be verified. Your messages are saved while recovery is checked.":companion.runtimeUpdateStatus==="failed"?"The previous version was restored. This Companion can keep working.":companion.runtimeUpdateStatus==="deferred"?"An update is waiting for a safe moment to preserve your work.":undefined} aria-current={section==="activity"?"page":undefined} onClick={()=>onSection("activity")}>
