...
import { productActivation, billingConfiguration, flushPendingUsage, handleBilling, handleStripeWebhook, migrateBilling, recordUsage, setBillingProviderForTests, stripeCheckoutForm, verifyStripeSignature, type BillingProvider, type UsageInput } from "../src/billing";
import { db } from "../src/store";

const saved = { ...process.env };
beforeAll(async () => { await migrate(); await migrateBilling(); });
afterEach(() => {
  for (const key of ["STRIPE_SECRET_KEY","STRIPE_BASE_PRICE_ID","STRIPE_MODEL_PRICE_ID","STRIPE_BOX_PRICE_ID","STRIPE_PRICE_ID","STRIPE_WEBHOOK_SECRET","STRIPE_METER_EVENT_NAME","STRIPE_BOX_METER_EVENT_NAME","APP_URL","BILLING_TEST_MODE"]) {
...
test("missing Stripe configuration remains visibly unavailable", async () => {
  for (const key of ["STRIPE_SECRET_KEY","STRIPE_BASE_PRICE_ID","STRIPE_MODEL_PRICE_ID","STRIPE_BOX_PRICE_ID","STRIPE_PRICE_ID","STRIPE_WEBHOOK_SECRET","STRIPE_METER_EVENT_NAME","STRIPE_BOX_METER_EVENT_NAME","APP_URL","BILLING_TEST_MODE"]) delete process.env[key];
...
test("Stripe mode requires three distinct prices and Checkout adds one base plus two metered items", () => {
  stripeMode();
  delete process.env.STRIPE_BASE_PRICE_ID;
  expect(billingConfiguration()).toMatchObject({ mode: "unconfigured", missing: ["STRIPE_BASE_PRICE_ID"] });
  process.env.BILLING_TEST_MODE = "1";
  expect(billingConfiguration().mode).toBe("test");
  delete process.env.BILLING_TEST_MODE;
  process.env.STRIPE_BASE_PRICE_ID = process.env.STRIPE_MODEL_PRICE_ID;
  expect(billingConfiguration()).toMatchObject({ mode: "unconfigured", duplicatePrices: true });
  process.env.STRIPE_BASE_PRICE_ID = "price_base_local";
  const form = stripeCheckoutForm({ ownerId: "owner", email: "owner@example.com", customerId: null });
  expect(form.get("line_items[0][price]")).toBe("price_base_local");
  expect(form.get("line_items[0][quantity]")).toBe("1");
  expect(form.get("line_items[1][price]")).toBe("price_model_local");
  expect(form.get("line_items[2][price]")).toBe("price_box_local");
...
test("late meter delivery preserves the usage time from the subscription period where it occurred", async () => {
  ...
  expect((await webhook("customer.subscription.updated", {
    ...
    cancel_at_period_end: false, items: { data: [{ price: { id: "price_base_local" } }, { price: { id: "price_model_local" } }, { price: { id: "price_box_local" } }] },
...
test("only the selected subscription and configured price grant access", async () => {
  ...
  const subscriptionObject = (id: string, status: string, prices = ["price_base_local", "price_model_local", "price_box_local"]) => ({
    id, customer, status, metadata: { owner_id: owner }, current_period_end: base + 3600,
    cancel_at_period_end: false, items: { data: prices.map(price => ({ price: { id: price } })) },
  });
  ...
  expect((await webhook("customer.subscription.updated", subscriptionObject(wrongPriceSubscription, "active", ["price_base_local", "price_other", "price_box_local"]), base + 6)).status).toBe(200);
  ...
  const exact = ["price_base_local", "price_model_local", "price_box_local"];
  const invalidSets = [
    ["price_model_local", "price_box_local"],
    [...exact, "price_extra"],
    [...exact, "price_box_local"],
  ];
  let created = base + 7;
  for (const invalid of invalidSets) {
    expect((await webhook("customer.subscription.updated", subscriptionObject(wrongPriceSubscription, "active", exact), created++)).status).toBe(200);
    expect(await (await handleBilling(new Request("http://localhost/api/billing"), owner))!.json()).toMatchObject({ active: true, plan: "subscription" });
    expect((await webhook("customer.subscription.updated", subscriptionObject(wrongPriceSubscription, "active", invalid), created++)).status).toBe(200);
    expect(await (await handleBilling(new Request("http://localhost/api/billing"), owner))!.json()).toMatchObject({ active: false, plan: "inactive" });
  }
});
...
test("raw Stripe signatures and durable event IDs protect subscription ownership", async () => {
  ...
  await db`INSERT INTO billing_accounts (owner_id,stripe_customer_id) VALUES (${owner},${customer})`;
  const payload = JSON.stringify({ id: `evt_${crypto.randomUUID()}`, type: "customer.subscription.updated", created: Math.floor(Date.now()/1000), data: { object: { id: subscription, customer, status: "active", metadata: { owner_id: owner }, current_period_end: Math.floor(Date.now()/1000)+3600, cancel_at_period_end: false, items: { data: [{ price: { id: "price_base_local" } }, { price: { id: "price_model_local" } }, { price: { id: "price_box_local" } }] } } } });
  ...
  const stalePayload = JSON.stringify({ id: `evt_${crypto.randomUUID()}`, type: "customer.subscription.updated", created: Math.floor(Date.now()/1000)-60, data: { object: { id: subscription, customer, status: "past_due", metadata: { owner_id: owner }, items: { data: [{ price: { id: "price_base_local" } }, { price: { id: "price_model_local" } }, { price: { id: "price_box_local" } }] } } } });
...
test("activation can share an existing transaction instead of acquiring a nested connection",async()=>{
 ...
 const owner=await user(`transaction-${crypto.randomUUID()}@example.com`),customer=`cus_${crypto.randomUUID()}`,subscription=`sub_${crypto.randomUUID()}`;
  ...
  await tx`INSERT INTO billing_subscriptions(stripe_subscription_id,owner_id,stripe_customer_id,stripe_price_id,subscription_status,last_event_created) VALUES(${subscription},${owner},${customer},${JSON.stringify(['price_base_local','price_box_local','price_model_local'])},'active',1)`;
  expect((await productActivation(owner,tx)).allowed).toBe(true);
  await tx`UPDATE billing_subscriptions SET stripe_price_id=${JSON.stringify(['price_box_local','price_model_local'])} WHERE stripe_subscription_id=${subscription}`;
  expect((await productActivation(owner,tx)).allowed).toBe(false);
  await tx`UPDATE billing_subscriptions SET stripe_price_id=${JSON.stringify(['price_base_local','price_box_local','price_model_local'])} WHERE stripe_subscription_id=${subscription}`;
  expect((await productActivation(owner,tx)).allowed).toBe(true);
