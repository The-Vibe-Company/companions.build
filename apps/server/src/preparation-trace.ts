...
const phases=['box_create','box_get','box_resume','box_setup','box_endpoint','box_environment','box_services','box_host','lifecycle_health','ready_checkpoint','admission_health','plugin_configuration','run_staging','admission_put'] as const;
...
function emit(companionId:string,phase:PreparationPhase,startedMs:number,outcome:Outcome,runId?:string){
 try{
  console.info(JSON.stringify({event:'preparation_trace',companionId,phase,
   startedMs:Math.round(startedMs*1000)/1000,durationMs:Math.round(Math.max(0,performance.now()-startedMs)*1000)/1000,
