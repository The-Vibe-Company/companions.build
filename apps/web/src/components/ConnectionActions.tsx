...
import { Ellipsis, ShieldCheck, Unplug } from "lucide-react";
import { Button } from "@/components/ui/button";
import "./ConnectionActions.css";

export function ConnectionActions({ label, busy, onCheck, onDisconnect }: {
  label: string;
  busy: boolean;
  onCheck?: () => void;
  onDisconnect: () => void;
  ...
  return <details className="connection-actions" ref={disclosure} open={open}
    ...
    {open && <div id={panelId} className="connection-actions-panel">
      {confirming ? <div className="connection-actions-confirm" role="group" aria-label={`Disconnect ${label}?`}>
